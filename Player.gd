...
export var x_mouse_sensitivity = .25
...
func queue_jump():
	...
	if Input.is_action_just_released("jump"):
		wish_jump = false
#	if Input.is_action_just_pressed("test_function"):
#		self.get_parent().testBreak()
...
func _input(ev):
	...
	if (ev is InputEventMouseButton):
		if ev.button_index==BUTTON_LEFT and ev.pressed==true:
			var newFireParticle = fireParticleScene.instance()
			add_child(newFireParticle)
			newFireParticle.translation = self.translation+Vector3(0,0,-2)
